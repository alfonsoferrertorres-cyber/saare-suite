...
export default function App() {
  ...
  const handleDownloadSealedPDF = (logItem) => {
    ...
    printWindow.document.write(\`<html><head><title>CERTIFICADO EVIDENCIA SAARE</title></head><body style="font-family:sans-serif;padding:30px;background:#f8fafc;color:#1e293b;"><h2>S.A.A.R.E. DICTAMEN DE EVIDENCIA INMUTABLE</h2><hr/><p><b>ID:</b> \${targetLog.id}</p><p><b>HECHO:</b> "\${targetLog.prompt}"</p><p><b>DICTAMEN:</b> \${targetLog.status}</p><p><b>EXPLICACIÓN:</b> \${targetLog.explanation}</p><div style="border:2px dashed #059669;background:#ecfdf5;padding:15px;margin-top:20px;text-align:center;"><b>SELLO CRIPTOGRÁFICO DE PRUEBA PLENA</b><br/><code style="font-size:11px;">SHA256-ED25519-\${targetLog.crypto_hash}-\${Date.now()}</code></div><script>window.onload=function(){window.print();}</script></body></html>\`);
  ...
  return (
    <div style={{ backgroundColor: '#e2e8f0', minHeight: '100vh', color: '#1e293b', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      {/* CABECERA CORPORATIVA CON LA NUEVA IMAGEN Y ESTILO GRIS */}
      <div style={{ width: '100%', height: '220px', backgroundImage: 'url(/saare-brand-header.jpg)', backgroundSize: 'contain', backgroundRepeat: 'no-repeat', backgroundPosition: 'center', backgroundColor: '#e2e8f0', borderBottom: '2px solid #cbd5e1', position: 'relative' }}>
      </div>

      <div style={{ padding: '28px', maxWidth: '1400px', margin: '0 auto' }}>
        
        {/* ENCABEZADO CONSOLA EN TEMA GRIS */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '2px solid #cbd5e1', paddingBottom: '16px', background: '#ffffff', padding: '16px 24px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
          <div>
        ...
        {/* PESTAÑA 2: RUNLIVE TELEMETRÍA GRIS */}
        {activeTab === 'RUNLIVE' && (
          <div>
            <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: '12px', padding: '24px', marginBottom: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: '12px', padding: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
              <h3 style={{ color: '#0f172a', margin: '0 0 12px 0', fontSize: '16px' }}>🏛️ DEMOSTRACIÓN DE LA EVIDENCIA GLOBAL E INTANGIBLE</h3>
              <p style={{ color: '#475569', fontSize: '14px', lineHeight: '1.7', margin: 0 }}>La evidencia {lastLog?.id} constituye una prueba plena de la interacción ocurrida. El registro ha sido fijado mediante un sello criptográfico inmutable, lo que garantiza la integridad absoluta de la información capturada, su trazabilidad legal y la imposibilidad de manipulación posterior del hecho probado.</p>
            </div>
          </div>
        )}

        {/* PESTAÑA 3: BIBLIOTECA DE ESCENAS */}
...
console.log('=== TEMA GRIS CORPORATIVO Y NUEVA CABECERA APLICADOS EN SAARE-CONSOLE ===');
