...
export default function App() {
  ...
  const handleDownloadSealedPDF = (logItem) => {
    ...
    printWindow.document.write(\`<html><head><title>CERTIFICADO EVIDENCIA SAARE</title></head><body style="font-family:sans-serif;padding:30px;background:#f8fafc;color:#1e293b;"><h2>S.A.A.R.E. DICTAMEN DE EVIDENCIA INMUTABLE GLOBAL</h2><hr/><p><b>ID:</b> \${targetLog.id}</p><p><b>HECHO:</b> "\${targetLog.prompt}"</p><p><b>DICTAMEN:</b> \${targetLog.status}</p><p><b>EXPLICACIÓN:</b> \${targetLog.explanation}</p><div style="border:2px dashed #059669;background:#ecfdf5;padding:15px;margin-top:20px;text-align:center;"><b>SELLO CRIPTOGRÁFICO DE PRUEBA PLENA Y VALIDEZ GLOBAL</b><br/><code style="font-size:11px;">SHA256-ED25519-\${targetLog.crypto_hash}-\${Date.now()}</code></div><script>window.onload=function(){window.print();}</script></body></html>\`);
  ...
  return (
    <div style={{ backgroundColor: '#e2e8f0', minHeight: '100vh', color: '#1e293b', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ padding: '28px', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        {/* PESTAÑA 2: RUNLIVE CON DEMOSTRACIÓN PROBATORIA GLOBAL */}
        {activeTab === 'RUNLIVE' && (
          <div>
            ...
            {/* SECCIÓN DEMOSTRACIÓN DE LA EVIDENCIA GLOBAL E INTANGIBLE */}
            <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: '12px', padding: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.04)' }}>
              <div style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '16px' }}>
                <h3 style={{ color: '#0f172a', margin: 0, fontSize: '17px', fontWeight: '800' }}>🏛️ DEMOSTRACIÓN DE LA EVIDENCIA GLOBAL E INTANGIBLE</h3>
                <p style={{ color: '#64748b', fontSize: '12px', margin: '4px 0 0 0' }}>Certificación de certeza plena e inalterabilidad del hecho interceptado ante cualquier jurisdicción.</p>
              </div>

              <p style={{ color: '#334155', fontSize: '14px', lineHeight: '1.7', margin: '0 0 20px 0' }}>
                La evidencia <strong style={{ color: '#0284c7' }}>{lastLog?.id || 'EV-864387'}</strong> constituye una prueba plena e inalterable de la interacción. El registro ha sido fijado mediante un sello criptográfico inmutable, lo que garantiza la integridad absoluta de la información capturada, su trazabilidad legal y la imposibilidad de manipulación posterior del hecho probado.
              </p>

              {/* TRES PILARES DE VALIDACIÓN GLOBAL */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
                
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '16px', borderRadius: '10px' }}>
                  <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#0284c7', marginBottom: '6px' }}>🌐 SELLADO DESCENTRALIZADO</div>
                  <div style={{ fontSize: '11px', color: '#64748b', lineHeight: '1.5' }}>
                    Huella digital generada mediante hash de alta entropía (ED25519) con sellado de tiempo inmutable.
                  </div>
                </div>

                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '16px', borderRadius: '10px' }}>
                  <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#16a34a', marginBottom: '6px' }}>🛡️ INVIOLABILIDAD L7</div>
                  <div style={{ fontSize: '11px', color: '#64748b', lineHeight: '1.5' }}>
                    Captura realizada en la capa de transporte previa al procesamiento, asegurando la custodia intacta del origen.
                  </div>
                </div>

                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '16px', borderRadius: '10px' }}>
                  <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#b45309', marginBottom: '6px' }}>📜 EFICACIA TRANSFRONTERIZA</div>
                  <div style={{ fontSize: '11px', color: '#64748b', lineHeight: '1.5' }}>
                    Alineación con directivas eIDAS (UE) y estándares ISO/IEC 27037 de preservación de evidencia digital.
                  </div>
                </div>
...
console.log('=== DEMOSTRACIÓN PROBATORIA GLOBAL INTEGRADA CON ÉXITO ===');
