...
export default function App() {
    ...
    try {
      ...
      if (data.events && data.events.length > 0) {
        ...
        const initialMock = [{
          ...
          cryptoSeal: 'SHA256-JAILBREAK-GUARD-2026'
        }];
        setLogs(initialMock);
        setLastLog(initialMock[0]);
      }
  ...
  // AUDITORÍA WEBCRYPTO EN NAVEGADOR
  ...
  // DESCARGA DE DICTAMEN PDF SELLADO
      ...
      <div style={{ padding: '28px', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', background: '#ffffff', padding: '16px 24px', borderRadius: '12px', border: '1px solid #cbd5e1' }}>
          <div>
            ...
            {activeCount > 0 ? MASTER PASS RUNTIME ACTIVO ( REGLAS) : RUNTIME SIN REGLAS ACTIVAS}
            {activeCount > 0 ? MASTER PASS RUNTIME ACTIVO ( REGLAS) : RUNTIME SIN REGLAS ACTIVAS}
          </div>
        </div>
        ...
        {activeTab === 'REGISTRO_GLOBAL' && (
          <div style={{ background: '#ffffff', borderRadius: '12px', border: '1px solid #cbd5e1', padding: '20px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
              ...
              <tbody>
                {logs.map(l => (
                  <tr key={l.evidenceId || l.runId} style={{ borderBottom: '1px solid #e2e8f0' }}>
        ...
        {/* PESTAÑA 2: RUNLIVE CON LOS BOTONES RESTAURADOS */}
        {activeTab === 'RUNLIVE' && (
          <div>
            <div style={{ background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid #e2e8f0', paddingBottom: '16px' }}>
                ...
                {/* RESTAURACIÓN DE BOTONES CLAVE DE AUDITORÍA Y PDF */}
              ...
              {/* MUESTRA DE VERIFICACIÓN WEBCRYPTO */}
              {verificationResult && (
              ...
              {/* INJECTOR PARA PRUEBAS EN TIEMPO REAL */}
        ...
        {/* PESTAÑA 3: BIBLIOTECA DE ESCENAS (CONMUTACIÓN VERDE/ROJO) */}
...
console.log('=== APLICACIÓN COMPLETA RESTAURADA CON BOTONES DE AUDITORÍA Y PDF ===');
