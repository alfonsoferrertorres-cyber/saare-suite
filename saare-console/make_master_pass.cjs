...
export default function App() {
  ...
  // Lista completa con Master Pass activo
  ...
  const activeScene = libraryScenes.find(s => s.id === activeSceneId) || libraryScenes[1];

  const handleActivateScenario = async (sc) => {
    if (!sc.licensed) return;

    try {
      await fetch('http://localhost:3001/api/v1/runtime/activate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ scenarioId: sc.id })
      });
    } catch (e) {
      console.log('Conmutación local realizada');
    }
      ...
      <div style={{ padding: '28px', maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', background: '#ffffff', padding: '16px 24px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', border: '1px solid #cbd5e1' }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#64748b' }}>ORGANIZACIÓN: <strong>ACME Corporation</strong> | FIRMA EN RUNTIME: <span style={{ color: '#16a34a', fontFamily: 'monospace', fontWeight: 'bold' }}>{activeScene.cryptoSignature}</span></p>
          </div>
          <div style={{ background: '#dcfce7', border: '1px solid #16a34a', color: '#15803d', padding: '8px 16px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>
            ● MASTER PASS ACTIVO | ACCESO TOTAL DESBLOQUEADO
          </div>
        </div>

        {/* PANEL DE LICENCIAMIENTO */}
        <div style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', color: '#ffffff', borderRadius: '12px', padding: '20px 24px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
          <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
            <div style={{ background: 'rgba(2, 132, 199, 0.2)', border: '1px solid #0284c7', padding: '10px 16px', borderRadius: '10px', textAlign: 'center' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: '900', color: '#ffffff' }}>🔑 ENTERPRISE SOVEREIGN (MASTER PASS)</span>
            </div>
            <div>
              <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 'bold', display: 'block' }}>COBERTURA DE LICENCIAS EN CONTROL-PLANE</span>
              <span style={{ fontSize: '16px', fontWeight: '800', color: '#4ade80' }}>4 de 4 Escenarios Core Habilitados <span style={{ fontSize: '12px', color: '#94a3b8', fontWeight: 'normal' }}>(Acceso Total)</span></span>
            </div>
          </div>
          <a href="http://localhost:5174" target="_blank" rel="noreferrer" style={{ background: '#0284c7', color: '#ffffff', textDecoration: 'none', padding: '10px 18px', borderRadius: '8px', fontSize: '12px', fontWeight: 'bold' }}>
            🏪 Gestionar Flota en Marketplace Suite (:5174) ➔
          </a>
        </div>
        ...
        {/* TARJETAS CON CAMBIO DINÁMICO DE ESTADOS (VERDE = ACTIVO EN RAM, AZUL = DISPONIBLE) */}
            ...
            {libraryScenes.map(sc => {
              ...
              return (
                <div key={sc.id} style={{ 
                  ...
                  borderRadius: '12px', 
                  padding: '24px', 
                  boxShadow: isActive ? '0 4px 14px rgba(22,163,74,0.15)' : '0 2px 8px rgba(0,0,0,0.04)',
                  ...
                  <button onClick={() => handleActivateScenario(sc)} style={{ 
                    ...
                    {isActive ? '✓ ESCENARIO EN EJECUCIÓN EN MEMORIA RAM' : '⚡ CONMUTAR Y ACTIVAR EN RUNTIME'}
                  </button>
                </div>
              );
            })}
...
console.log('=== ACCESO COMPLETO UNIFICADO Y CAMBIO DINÁMICO VERDE/AZUL APLICADO ===');
