...
impl PerimeterShieldEngine {
    pub fn bootstrap(token_json: &str) -> Result<Self, PerimeterError> {
        ...
        // 1. Instanciación de LicenseVerifier (sin argumentos)
        let verifier = LicenseVerifier::new()
            .map_err(|_| PerimeterError::InvalidSignature)?;

        // 2. Conversión del payload a UTF-8 &str y de la firma a formato Hexadecimal
        let payload_str = std::str::from_utf8(&payload_bytes)
            .map_err(|_| PerimeterError::SerializationError)?;
        let signature_hex = hex::encode(signature_array);

        // 3. Verificación pasando los tipos &str esperados
        if !verifier.verify(payload_str, &signature_hex) {
...
mod tests {
    ...
    fn generate_mock_token(modules: Vec<String>) -> SignedLicenseToken {
    ...
    #[test]
    fn test_perimeter_unauthorized_key_rejection() {
        // Al firmar con una clave aleatoria (no oficial), la verificación debe fallar con InvalidSignature
        let token = generate_mock_token(vec!["01_perimetershield".to_string()]);
    ...
    #[test]
    fn test_perimeter_tampered_signature_rejection() {
        // Al alterarse los bytes de la firma, el sistema debe rechazar la transacción
        let mut token = generate_mock_token(vec!["01_perimetershield".to_string()]);
