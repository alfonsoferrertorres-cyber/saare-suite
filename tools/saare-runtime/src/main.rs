...
use config::*;
use license::{LicenseError, LicenseGuard};
use std::env;

const SAARE_SECRET_KEY: &str = "SAARE_ENTERPRISE_SECRET_KEY_2026";
const CURRENT_SIMULATED_EPOCH: u64 = 1770000000; // Fecha de ejecución válida (2026)
...
async fn main() {
    ...
    let mut target_module = String::from("09_nexus");
    let mut target_preset = String::from("default");
    let mut license_token = env::var("SAARE_LICENSE_KEY").unwrap_or_default();
    ...
    // Si no se provee licencia por CLI ni ENV, se genera un token demo para el modo de prueba
    if license_token.is_empty() {
        println!("[LICENCIA] No se detectó clave. Generando licencia temporal de desarrollo...");
        license_token = guard.issue_token("DEV_TENANT", 2000000000, &["*"]);
    }

    // Verificar derecho de uso antes de despachar cualquier preset
    match guard.verify_token(&license_token, &target_module, CURRENT_SIMULATED_EPOCH) {
        Ok(payload) => {
            println!("[LICENCIA VERIFICADA]");
            println!("  -> Cliente Autorizado: {}", payload.tenant_id);
        ...
        Err(err) => {
            ...
            match err {
                LicenseError::InvalidSignature => {
                    println!("  -> Firma criptográfica no válida o token manipulado.")
                }
                LicenseError::Expired => println!("  -> La licencia ha expirado."),
                LicenseError::ModuleNotAuthorized(m) => {
                    println!("  -> El módulo '{}' no está incluido en su contrato.", m)
                }
                LicenseError::InvalidFormat => {
                    println!("  -> Formato de token de licencia no válido.")
                }
            }
...
fn dispatch_perimeter(preset: &str) {
    ...
    println!(
        "  -> Financial Masking (DORA/PCI-DSS): {}",
        c.mask_financial_data
    );
...
fn dispatch_compliance(preset: &str) {
    ...
    println!(
        "  -> ISO 42001 Continuous Matrix: {}",
        c.continuous_evidence_matrix
    );
}
