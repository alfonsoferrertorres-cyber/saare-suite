  ...
  const fetchData = async () => {
    try {
      ...
      if (rScen && rScen.ok) {
        ...
        if (Array.isArray(dScen) && dScen.length > 0) setScenarios(dScen);
      }
      if (rRules && rRules.ok) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#94a3b8', fontFamily: fontStack, color: '#0f172a', paddingBottom: '40px', margin: 0 }}>
      ...
      <div style={{ width: '100%', background: '#64748b', overflow: 'hidden', borderBottom: '2px solid #475569' }}>
        <img src="/CABECERA WEB.jfif" alt="Cabecera SAARE" style={{ width: '100%', maxHeight: '190px', objectFit: 'cover', display: 'block' }} onError={(e) => { e.target.src = '/cabecera.jfif'; }} />
      </div>
      ...
      <main style={{ maxWidth: '1240px', margin: '22px auto 0 auto', padding: '0 20px' }}>
        ...
        {activeTab === 'library' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {/* CONFIGURADOR DE BLOQUEOS PERSONALIZADOS */}
            <div style={{ background: '#ffffff', padding: '20px 24px', borderRadius: '8px', border: '1px solid #64748b', boxShadow: '0 4px 8px rgba(0,0,0,0.08)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                ...
                <button
                  ...
                  style={{ background: '#0284c7', color: '#ffffff', border: 'none', padding: '10px 18px', borderRadius: '5px', fontWeight: 900, fontSize: '0.82rem', cursor: 'pointer', textTransform: 'uppercase' }}
                >
                  + Añadir Bloqueo
                </button>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '18px' }}>
              {scenarios.map(sc => {
                ...
                return (
                  <div key={sc.id} style={{ background: '#ffffff', border: `2px solid ${isEnabled ? '#16a34a' : '#dc2626'}`, borderRadius: '8px', padding: '20px', boxShadow: '0 4px 8px rgba(0,0,0,0.08)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                      ...
                      <span style={{ background: isEnabled ? '#dcfce7' : '#fee2e2', color: isEnabled ? '#15803d' : '#dc2626', border: `1px solid ${isEnabled ? '#16a34a' : '#dc2626'}`, padding: '3px 8px', borderRadius: '3px', fontSize: '0.72rem', fontWeight: 900, textTransform: 'uppercase' }}>
                    ...
                    <button
                      ...
                      style={{ width: '100%', padding: '11px', background: isEnabled ? '#16a34a' : '#dc2626', color: '#ffffff', border: 'none', borderRadius: '4px', fontWeight: 900, fontSize: '0.82rem', cursor: 'pointer', letterSpacing: '0.5px', textTransform: 'uppercase' }}
                    >
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: '#ffffff', borderRadius: '8px', padding: '24px', maxWidth: '500px', width: '90%', border: '2px solid #0284c7' }}>
            ...
            <p style={{ fontSize: '0.82rem', color: '#64748b', margin: '0 0 16px 0' }}>
              Introduce la sintaxis o expresión regular que deseas interceptar en origen.
            </p>
      ...
      {scenarioToDisable && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: '#ffffff', borderRadius: '8px', padding: '24px', maxWidth: '480px', width: '90%', border: '2px solid #dc2626' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              <button onClick={() => setScenarioToDisable(null)} style={{ padding: '8px 16px', background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: '4px', fontWeight: 800, fontSize: '0.8rem', cursor: 'pointer' }}>
                Cancelar
              </button>
              <button onClick={confirmDisable} style={{ padding: '8px 16px', background: '#dc2626', color: '#ffffff', border: 'none', borderRadius: '4px', fontWeight: 900, fontSize: '0.8rem', cursor: 'pointer' }}>
